...
class Cross_Entropy:
    ...
    def select_elites(self, states_batch, actions_batch, rewards_batch,
        ...
        elite_states = [s for i in range(len(states_batch)) if
                        rewards_batch[i] >= reward_threshold for s in
                        states_batch[i]]
        elite_actions = [a for i in range(len(actions_batch)) if
                         rewards_batch[i] >= reward_threshold for a in
                         actions_batch[i]]
    ...
    def update_policy(self, elite_states, elite_actions):
        ...
        for s in range(self.n_states):
            ...
            if (s in elite_states):
                ...
                list_actions = [elite_actions[i] for i in
                                range(len(elite_actions)) if
                                elite_states[i] == s]

                list_actions.sort()

                set_of_actions = set(list_actions)
                freq = [len(list(group)) for key, group in
                        groupby(list_actions)]

                for i in range(len(set_of_actions)):
                    new_policy[s][list(set_of_actions)[i]] = freq[
                                                                 i] / num_visits
    ...
    def loop(self, n_epochs, n_sessions, percentile, learning_rate):
        ...
        for i in range(n_epochs):
            sessions = [self.generate_session(policy) for i in
                        range(n_sessions)]

            states_batch, actions_batch, rewards_batch = zip(*sessions)

            elite_states, elite_actions = self.select_elites(states_batch,
                                                             actions_batch,
                                                             rewards_batch,
                                                             percentile)
